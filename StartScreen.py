...
class Start(Frame):
    ...
    def create_widgets(self):
        ...
        self.tpButton = Button(self, text="2 Player", bg="blue", font = 24,command=self.startgame2P)
        self.tpButton.grid(row=4, column=8, sticky=N)
        self.AIButton = Button(self, text="1 Player", bg="green", font=24, command=self.startgame1P)
        self.AIButton.grid(row=3, column=8, sticky=N)
        self.multi = Button(self,text="4 Player",bg="orange",font=24,command=self.startgame4p)
        self.multi.grid(row=5,column=8,sticky=N)
        self.creditButton = Button(self, text="Credits", bg="red", font=24, command=self.credits)
        self.creditButton.grid(row=6, column=8, sticky=N)
